class Bishop extends Piece {
  ...
  Bishop(Piece bishop) { //we know for sure that the piece object will be a bishop, but we still need to treat it as a piece
    super(bishop);
    this.pieceValue = bishop.pieceValue;
    squaresCopy[x][y] = this;
  }
  
  Bishop(PVector boardPos_, boolean isWhite_) {
  ...
  void display() {
    image(imgs[(1-int(isWhite))*6+1], pxPos.x, pxPos.y);  
  }
